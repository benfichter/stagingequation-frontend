            ...
            {orders.map((order) => {
              ...
              return (
                <Link key={order.id} href={`/orders/${order.id}`} className="block">
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 border rounded-lg p-4 transition hover:border-primary/40 hover:bg-muted/20">
                    <div className="space-y-1">
                      ...
                      <p className="text-sm">
                        {order.image_count} image{order.image_count === 1 ? "" : "s"} | ${total}{" "}
