  ...
  const handleConfigSubmit = async () => {
    if (!user) {
      toast({
        ...
        description: "Log in before running the demo.",
        variant: "destructive",
      });
      setCurrentStep(1);
      return;
    ...
    <div className="min-h-screen bg-background">
      ...
      {!user ? (
        <main className="max-w-4xl mx-auto px-4 py-16">
          <Card>
            <CardHeader>
              <CardTitle>Log in to run a demo</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4 text-sm text-muted-foreground">
              <p>
                Your demo workflow is separate from onboarding. Log in or create an account on the
                dashboard to start staging.
              </p>
              ...
              <div className="max-w-4xl mx-auto">
                {isProcessing ? (
                  <HouseLoadingAnimation
                    message="Staging your room and preparing calibration..."
                    duration={3000}
                  />
